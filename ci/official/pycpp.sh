...
if [[ $(uname -s) == "MSYS_NT"* ]] || [[ $(uname -s) == "MINGW64_NT"* ]]; then
  if [[ "$TFCI_PYCPP_DISABLE_DEF_FILE_GEN" != 1 ]]; then
    ...
    python3 ./tensorflow/tools/def_file_gen/regenerate_win_exports.py --stage=discovery --bust_cache --output_def_file=tensorflow/python/_pywrap_tensorflow_unmangled.def
    if [[ $? -eq 1 ]]; then
