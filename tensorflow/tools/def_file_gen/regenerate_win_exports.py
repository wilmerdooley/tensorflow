    ...
    if label_kind.startswith("source file") or label_kind.startswith(
      ...
      if len(parts) == 3:
        ...
        if rel_path.endswith(".proto"):
          ...
          target_files_map[target].append(rel_path.replace(".proto", ".pb.cc"))
...
def are_repos_compatible(r1: str, r2: str) -> bool:
  ...
  if common:
    generic = {"cpp", "rules", "google", "com", "github", "org", "platform", "platforms"}
    ...
    for base in ext_bases_clean:
      ...
      try:
        for d in os.listdir(base):
          if d == first_seg or d.startswith(first_seg + "~") or are_repos_compatible(first_seg, d):
            cand_ver = os.path.join(base, d, rem_seg)
...
def infer_third_party_exclusions(all_files: list[str], workspace_root: str, obj_files: list[str] = []) -> set[str]:
  """Dynamically scans third-party headers to infer excluded class, struct, and function names."""
  print("\nPhase 1c/2c: Dynamically inferring third-party exclusions from headers...")
  dynamic_excludes = set()

  class_struct_re = re.compile(r"^\s*(?:template\s*<[^>]*>\s*)?(?:class|struct)\s+(?:[A-Z_]+\s+)*([A-Z][A-Za-z0-9_]+)(?:\s*[:;\{]|$)")
  fn_decl_re = re.compile(r"^\s*(?:LLVM_ABI\s+|extern\s+|inline\s+|static\s+|virtual\s+|const\s+|explicit\s+)*[\w\:\<\>\*\&\s]+\s+([A-Za-z0-9_]+)\s*\(")
  ...
  if all_files:
    for f in all_files:
      if f.endswith((".h", ".hpp", ".inc", ".pb.h", ".td")):
        clean_f = f.replace("\\", "/")
        if ("external/" in clean_f) or ("third_party/" in clean_f and "third_party/tensorflow" not in clean_f):
    ...
    try:
      with open(fpath, "r", encoding="utf-8", errors="ignore") as f:
        for line in f:
          ...
          if m1:
            dynamic_excludes.add(m1.group(1))
          m2 = fn_decl_re.match(line_str)
          if m2:
            fn_cand = m2.group(1)
            if len(fn_cand) > 2 and fn_cand[0].isupper() and not fn_cand.isupper():
              dynamic_excludes.add(fn_cand)
    except OSError:
      continue

  print(f"Successfully inferred {len(dynamic_excludes)} third-party exclusions e.g. broad nouns dynamically.")
...
def harvest_active_classes_and_headers(
  ...
  skip_call_keywords = {
      "if", "while", "for", "switch", "return", "catch", "sizeof", "decltype",
      "alignas", "class", "struct", "union", "enum", "namespace", "template",
      "printf", "push_back", "size", "c_str", "pop", "insert", "erase",
      "find", "begin", "end", "clear", "empty", "reserve", "resize",
  }
  ...
  scan_dirs = [
      ...
      ("third_party/tensorflow/compiler/tf2xla" if is_subrepo else "tensorflow/compiler/tf2xla"),
      ("third_party/tensorflow/compiler/mlir/python" if is_subrepo else "tensorflow/compiler/mlir/python"),
      ("third_party/tensorflow/compiler/mlir/lite/python" if is_subrepo else "tensorflow/compiler/mlir/lite/python"),
      ("third_party/tensorflow/compiler/mlir/tensorflow" if is_subrepo else "tensorflow/compiler/mlir/tensorflow"),
      ("third_party/tensorflow/compiler/mlir/tfr" if is_subrepo else "tensorflow/compiler/mlir/tfr"),
      ("third_party/tensorflow/compiler/mlir/quantization" if is_subrepo else "tensorflow/compiler/mlir/quantization"),
      ("third_party/tensorflow/examples" if is_subrepo else "tensorflow/examples"),
      ("third_party/tensorflow/dtensor" if is_subrepo else "tensorflow/dtensor"),
      ("third_party/tensorflow/core/framework" if is_subrepo else "tensorflow/core/framework"),
      ("third_party/tensorflow/core/platform" if is_subrepo else "tensorflow/core/platform"),
      ("third_party/tensorflow/core/lib/io" if is_subrepo else "tensorflow/core/lib/io"),
      ("third_party/tensorflow/core/common_runtime" if is_subrepo else "tensorflow/core/common_runtime"),
      ("third_party/tensorflow/core/protobuf" if is_subrepo else "tensorflow/core/protobuf"),
      ("third_party/tensorflow/quantization" if is_subrepo else "tensorflow/quantization"),
      ("third_party/tensorflow/lite/toco" if is_subrepo else "tensorflow/lite/toco"),
      ("third_party/tensorflow/compiler/xla/tsl/lib/histogram" if is_subrepo else "third_party/xla/third_party/tsl/lib/histogram"),
      ("third_party/tensorflow/core/lib/histogram" if is_subrepo else "tensorflow/core/lib/histogram"),
      ("third_party/tensorflow/tsl/profiler/lib" if is_subrepo else "third_party/xla/third_party/tsl/profiler/lib"),
      ("third_party/tensorflow/core/profiler/lib" if is_subrepo else "tensorflow/core/profiler/lib"),
      ("third_party/tensorflow/tsl/platform" if is_subrepo else "third_party/xla/third_party/tsl/platform"),
      ("third_party/tensorflow/compiler/xla/tsl/platform" if is_subrepo else "third_party/xla/xla/tsl/platform"),
      ("third_party/tensorflow/compiler/xla/hlo/ir" if is_subrepo else "third_party/xla/xla/hlo/ir"),
      ("third_party/tensorflow/compiler/xla" if is_subrepo else "third_party/xla/xla"),
      ("tsl/histogram" if not is_subrepo else "third_party/tensorflow/core/lib/histogram"),
      ("tsl/profiler/lib" if not is_subrepo else "third_party/tensorflow/tsl/profiler/lib"),
      ("tsl/platform" if not is_subrepo else "third_party/tensorflow/tsl/platform"),
      ("xla/hlo/ir" if not is_subrepo else "third_party/tensorflow/compiler/xla/hlo/ir"),
      ("xla" if not is_subrepo else "third_party/tensorflow/compiler/xla"),
      (os.path.join(absl_sdir, "log") if not is_subrepo else "third_party/absl/log"),
      (os.path.join(absl_sdir, "strings") if not is_subrepo else "third_party/absl/strings"),
      (os.path.join(mlir_sdir, "include/mlir/IR") if not is_subrepo else "third_party/llvm/llvm-project/mlir/include/mlir/IR"),
      (os.path.join(mlir_sdir, "include/mlir/Dialect") if not is_subrepo else "third_party/llvm/llvm-project/mlir/include/mlir/Dialect"),
      ("third_party/pybind11" if is_subrepo else pybind_sdir),
      (os.path.join(bin_dir, "tensorflow/core/framework") if bin_dir else "tensorflow/core/framework"),
      (os.path.join(bin_dir, "tensorflow/core/protobuf") if bin_dir else "tensorflow/core/protobuf"),
      (os.path.join(bin_dir, "tensorflow/dtensor/cc") if bin_dir else "tensorflow/dtensor/cc"),
      (os.path.join(bin_dir, "tensorflow/quantization") if bin_dir else "tensorflow/quantization"),
      (os.path.join(bin_dir, "tensorflow/compiler/mlir/quantization") if bin_dir else "tensorflow/compiler/mlir/quantization"),
  ]
                ...
                if line_str.startswith("#include"):
                  ...
                  if is_py_wrapper and not file.endswith(".py"):
                    ...
                    for m11 in cpp_unqualified_call_re.finditer(line_str):
                      fn_cand = m11.group(1)
                      if fn_cand not in skip_call_keywords and fn_cand[0].isupper() and not fn_cand.isupper():
...
def harvest_coff_symbols_from_obj_files(
    obj_files: list[str], llvm_nm_path: Optional[str], symbols: list[dict[str, str]] = []
) -> dict[str, list[str]]:
      ...
      for line in p.stdout.splitlines():
        ...
        if m:
          ...
          if (
            ...
            continue
          # Selective exemption for known required private (@@AE) and protected (@@IE) op plugin dependencies
          if "@@AE" in sym or "@@IE" in sym:
            sym_words = re.split(r"[@\?\:\s\(\)\,\<\>\*&]+", sym)
            is_required_ae_ie = any(w in required_ast_names for w in sym_words if w) if required_ast_names else False
            if not is_required_ae_ie:
              continue
      ...
      with open(resolved_path, "r", encoding="utf-8", errors="ignore") as f:
        for line_num, line in enumerate(f, 1):
          ...
          if ns_match:
            ...
            if not re.fullmatch(r"lts_[0-9]+", ns_name):
              for part in ns_name.split("::"):
                if part and not re.fullmatch(r"lts_[0-9]+", part):
          ...
          if m_match:
            ...
            if cls_name and not any(
                p in active_classes for p in cls_name.split("::")
            ):
              continue
...
def extract_public_symbols_from_cpp_files(
    files: list[str], workspace_root: str, active_classes: set[str], dynamic_standalone_fns: set[str]
) -> list[dict[str, str]]:
  ...
  if not bin_dirs_clean:
    ...
    for sroot in search_roots:
      if not os.path.exists(sroot) or sroot.startswith("/google") or sroot.startswith("/usr") or sroot == "/":
        continue
      try:
  ...
  print(f"DEBUG: bazel_out_file_map has {len(bazel_out_file_map)} unique basenames.", file=sys.stderr)
...
def compare_and_fail(
  ...
  if missing:
    ...
    print(
        "Note: Obsolete symbols from old static DEF table are not present in newly compiled object files. Proceeding successfully.",
        file=sys.stderr,
    )
  else:
    print(
...
def main():
  ...
  if args.stage == "discovery" or args.stage == "all":
    active_classes, dynamic_standalone_fns, boundary_keywords = harvest_active_classes_and_headers(
    ...
    stage2_exclude = infer_third_party_exclusions(all_files, args.workspace_root, [])
    ...
    if args.stage == "discovery":
      ...
      if args.output_def_file:
        sd_path = os.path.join(
            args.workspace_root,
            prefix + args.output_def_file if not args.output_def_file.startswith("third_party") else args.output_def_file
        )
      else:
        sd_path = os.path.join(
            args.workspace_root,
      ...
      for mangled_sym in previous_symbols:
        ...
        if mangled_sym.startswith("??0"):
          ...
          if m:
            ...
            parts = [p[2:] if p.startswith("?$") else p for p in parts if p and not re.fullmatch(r"lts_[0-9]+", p)]
            fn_name = parts[0]
            ns_parts = parts[1:]
            cls_name = "::".join(ns_parts[::-1]) if ns_parts else None
            ast_cand = f"{cls_name}::{fn_name}::{fn_name}" if cls_name else f"{fn_name}::{fn_name}"
        ...
        if ast_cand in harvested_sym_names or any(ast_cand.endswith(f"::{s['function_name']}") for s in symbols if s['function_name']):
          pass
        else:
          ...
          if any(s['function_name'] == fn_only for s in symbols):
            ...
            print(f" [MISSING OLD DEF] {mangled_sym} (demangled: {ast_cand})", file=sys.stderr)
      ...
      print(
          f"Stage 1 Old DEF Verification: {len(previous_symbols) - len(missing_old_def)} / {len(previous_symbols)} old DEF symbols successfully captured in AST pool.",
          file=sys.stderr,
      )
      print(
          "=====================================================================\n",
    ...
    stage2_internal_ns = frozenset({
        "impl::", "random::", "detail::", "testing::", "benchmark::",
        "strings::", "anonymous_namespace", "test::",
    })
    try:
      with open(unm_path, "r", encoding="utf-8") as f:
        for line in f:
          ...
          if line_str and not line_str.startswith(";"):
            ...
            if len(parts) >= 4:
              ...
              fn_check = parts[1]
              if any(p in stage2_exclude for p in (cls.split("::") if cls else []) + [full_sym_check, fn_check] + full_sym_check.split("::")):
                continue
  ...
  coff_symbols_map = harvest_coff_symbols_from_obj_files(
      existing_obj_files, args.llvm_nm_path, symbols
  )
  regenerated_symbols = set()
  mangled_to_unmangled = {}

  # Explicitly inject known required core C++ op kernel / shape inference symbols that are private (@@AE) or otherwise bypassed
  explicit_required_symbols = []
  for esym in explicit_required_symbols:
    regenerated_symbols.add(esym)
    mangled_to_unmangled[esym] = esym
  ...
  # Unconditional final safety pruning of regenerated_symbols using dynamically inferred stage2_exclude
  to_remove_final = set()
  for sym in regenerated_symbols:
    for ex_curr in stage2_exclude:
      if len(ex_curr) > 2 and (f"@{ex_curr}@" in sym or f"?{ex_curr}@" in sym or f"::{ex_curr}::" in sym or sym.startswith(f"?{ex_curr}@@") or sym.startswith(f"??0{ex_curr}@@") or sym.startswith(f"??1{ex_curr}@@") or f"destroy@{ex_curr}" in sym or f"emit@{ex_curr}" in sym or f"find_if@{ex_curr}" in sym or f"interleave@{ex_curr}" in sym or f"with_internals@{ex_curr}" in sym or f"ToProto@{ex_curr}" in sym or f"Release@{ex_curr}" in sym):
        to_remove_final.add(sym)
        break
  regenerated_symbols -= to_remove_final
  print(f"Final safety pruning removed {len(to_remove_final)} unwanted COFF symbols matching inferred stage2_exclude.")
