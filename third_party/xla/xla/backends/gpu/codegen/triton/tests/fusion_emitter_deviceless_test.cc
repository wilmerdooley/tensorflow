...
fusion {
  ...
  bitcast = f32[5,2] bitcast(slice)
  ROOT dot = f32[2,20] dot(bitcast, p1),
...
TEST_F(TritonEmitterDevicelessTest, ScanEmitOk) {
  const std::string kHloText = R"(
scan_computation {
...
// CHECK:           %[[SCAN:.*]] = "tt.scan"(%[[EXTRACT]])
// CHECK-SAME:      axis = 0 : i32, reverse = false
